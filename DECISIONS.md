...
### D-073 — SFT raised capability more than reliability, and widened the gap this project studies
**Date:** 2026-08-21 · **Status:** accepted · **Supersedes:** the statistics in `results/sft-comparison-qwen3-1.7b-c16530e.json`
**Label:** `post_hoc_after_measurement` · **Evidence class:** labelled auxiliary decomposition, never confirmatory (§1.1)

The first trained arm. `Qwen/Qwen3-1.7B` fine-tuned on 684 rejection-sampled
single-turn trajectories generated by `Qwen/Qwen3-4B` over the frozen train
split, checkpoint-75 selected on dev by the rule pinned before any dev number
existed, evaluated on the 150-task test split exactly once.

| rung | metric | base → SFT | difference | 95% CI | exact sign p | changed |
| --- | --- | --- | --- | --- | --- | --- |
| R0 | pass^1 | 0.3033 → 0.5250 | +0.2217 | [+0.1550, +0.2900] | 2.9e-09 | 74/150 |
| R0 | pass^4 | 0.2467 → 0.3933 | +0.1467 | [+0.0667, +0.2267] | 6.8e-04 | 40/150 |
| R1 | pass^1 | 0.3333 → 0.5300 | +0.1967 | [+0.1250, +0.2667] | 3.8e-08 | 75/150 |
| R1 | pass^4 | 0.2867 → 0.3933 | +0.1067 | [+0.0267, +0.1933] | 1.95e-02 | 42/150 |

**The headline is capability, not reliability, and the distinction is the
finding.** `pass@4` moved 0.353 → 0.680 while `pass^4` moved 0.247 → 0.393, so
the band of tasks solved sometimes but not always **widened from 0.107 to
0.287**. The model now solves far more problems on some attempt and is
relatively less able to repeat it. Calling this a reliability win would invert
what the numbers say, and the gap is the thing this project exists to measure.

**It is not "tool-use reliability" either.** The schema-valid call rate was
**1.000 in both arms**; there was no headroom in tool use at all. Decomposed,
about +0.199 of the pass^1 gain is arithmetic correctness inside an
already-well-formed call and about +0.023 is emitting a call at all. The
correct name for what improved is execution-graded task accuracy.

**The confound is closed by measurement, not by argument.** The baseline was
recorded at commit `3cc174f` and the SFT arm at `c16530e`, with a runner
refactor in between. Re-running the untrained baseline on the current code
reproduces D-068 to four decimals on all six figures — R0 0.3033 / 0.2467 /
0.0150 and R1 0.3333 / 0.2867 / 0.0167 — so the harness is bit-stable under its
seeds and none of the difference is the refactor.

**A caveat that also attaches retroactively to D-068.** The untrained 1.7B
emitted four byte-identical completions on **82 of 150 tasks** (mean 1.73
distinct); 33 of its 37 `pass^4` successes came from those. After training it
is 7 of 150 (mean 3.04). Where an arm does not vary, `pass^k` collapses to
`pass^1` and measures nothing about repeatability, which is exactly what
`configs/eval.yaml` says temperature 0.7 was chosen to prevent. Restricted to
the 65 tasks that genuinely varied in both arms the gap **widens** to +0.2154,
so the finding survives; but every `pass^4` figure this project publishes must
carry the degeneracy rate beside it.

**Laundering fell on every definition, and the weak one was being reported.**
`evaluation.rungs._laundered` implements only the first of the project's three
retention rules, so the `no_arithmetic_rate` printed beside every accuracy
figure understates the rate that `training.retention` calls laundering.
Measured both ways over 600 R0 episodes per arm:

| definition | base | SFT |
| --- | --- | --- |
| harness rule 1 only | 9 (0.0150) | 1 (0.0017) |
| …and graded correct | 3 (0.0050) | 0 (0.0000) |
| retention rules 1–3 | 18 (0.0300) | 5 (0.0083) |
| …and graded correct | 11 (0.0183) | 2 (0.0033) |

The direction is the same on all four rows, so the conclusion holds: training
did not buy accuracy by handing the calculator an answer the model already had.
The word "recall" must not be used for it, because the token-starved recall
probe of D-064 has not been re-run on the trained checkpoint.

**Corrections to this project's own analysis, found by adversarial review.**
`scripts/compare_arms.py` reported `p_value_two_sided: 0.0`, which is a
resample count rather than a probability; used 20,000 resamples where
`HYPOTHESIS_PROTOCOL` §6.1 and §6.2 pin 10,000; and reported no interval at
all, though §6.2 states permutation tests are secondary to estimates and CIs.
All three are fixed and the comparison re-run. The earlier artifact is left in
place because measurement records are immutable (D-052), and is superseded
here rather than edited.

**What this does not support.** H1 remains **NA**, not confirmed. It requires a
GRPO arm, a primary-model arm, the 8B scaffolded comparator (registered but
never measured, so `Gap_H1` is uncomputable), and per-episode generated-token
accounting that the evaluator does not yet produce, so `TokenRatio_H1` is
unmeasurable with current code. §1.2 also requires n=8 for a confirmatory arm
and this is n=4. The teacher is Qwen3-4B rather than §5.2's 32B class (D-072),
and only the scale-check model was trained, so nothing here speaks to the
primary model.

**Also honest about power.** This is one training seed (20260820) with no
training-seed variance estimate, so +0.2217 is one run rather than an
established effect size. The quoted intervals and p-values bound evaluation
noise on a fixed checkpoint only. Checkpoint-75 beat checkpoint-86 on dev by
0.005, which is two episodes out of 400. Of the four comparisons reported, both
R0 rows clear a Bonferroni threshold of 0.0125 and **R1 pass^4 does not**; no
multiplicity policy was pre-registered, and the threshold is reported rather
than used to filter what is shown.

**One prediction confirmed.** D-068 argued the R1 rung could only ever repair
